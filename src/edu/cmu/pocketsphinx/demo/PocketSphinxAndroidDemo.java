...
public class PocketSphinxAndroidDemo extends Activity implements
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		CharSequence title = "Mikes Speech Time Clock";
		this.setTitle(title);
		...
		// this.rec = new
		// RecognizerTask(defaultConfig[0],defaultConfig[1],defaultConfig[2]);
		...
		words.setOnFocusChangeListener(new View.OnFocusChangeListener() {
			...
			public void onFocusChange(View v, boolean hasFocus) {
				if (hasFocus) {
					// change thte config
					final Toast toast = Toast.makeText(context, "Words",
							duration);
					toast.show();
					setConfiguration(1);
					activeField = words;
		...
		this.feedback = (EditText) findViewById(R.id.EditText02);
	...
	public void onPartialResults(Bundle b) {
		...
		that.activeField.post(new Runnable() {
			public void run() {
				...
				if (that.activeFieldStr.equals("number")
						|| that.activeFieldStr.equals("digit")) {
	...
	public void setConfiguration(int type) {
		...
		this.rec = new RecognizerTask(defaultConfig[0], defaultConfig[1],
				defaultConfig[2]);
		this.rec_thread = new Thread(this.rec);
		this.listening = false;
		this.rec.setRecognitionListener(this);
	...
	protected void onActivityResult(int requestCode, int resultCode,
		...
		Log.d("PocketSphinx.Main", "result code: " + resultCode + " from "
				+ requestCode);
		if (resultCode == RESULT_OK) {
			...
			case 0:
				try {
					...
					// rec_thread.destroy();
					// }
					Log.d("PocketSphinx.Main", "result code: " + resultCode
							+ " from " + requestCode);
					String[] defaultConfig = getConfiguration();
					// Log.d("PocketSphinx.Main","configuration is " +
					// defaultConfig[0]);
					// RecognizerTask(String hmm, String lm, String dict)
					this.rec = new RecognizerTask(defaultConfig[0],
							defaultConfig[1], defaultConfig[2]);
	...
	protected static String convertWordsToNumbers(String hyp) {
		...
		String[] wordNum = SegmentNumber.segmentNum(hyp.toLowerCase())
				.replace(" | ", "%").split("%");
		...
		for (String word : wordNum) {
			try {
				...
				if (word.equals(""))
					continue;
				String dig = ConvertWordToNumber
						.WithSeparator(ConvertWordToNumber.parse(word));
